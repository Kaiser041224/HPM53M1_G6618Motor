...
static uint8_t Cali_Run(void)
{
  ...
  }
  {
    ...
    (void)Encoder2_ReadFrame(&aux0, 0);
    Cali_Delay(20U);
    (void)Encoder_ReadFrame(&raw1, 0);
    Dbg_Printf("cali: raw=%u %u aux=%u th0=%d\r\n",
               (unsigned)raw0,
               (unsigned)raw1,
               (unsigned)aux0,
               (int)(theta0 * 1000.0f));
  ...
  if (Servo_SeedMultiTurn(Servo_GetZeroMotor()) == 0U)
  {
...
static uint8_t Cali_ApplySaved(const CaliNvData_t *nv)
{
  ...
  if (Servo_SeedMultiTurn(nv->mt_zero_rad) == 0U)
  {
    Foc_EncoderReset(enc, raw_mech);
    Foc_EncoderSetZeroAbs(enc, nv->mt_zero_rad);
    Dbg_Printf("cali: vernier fallback\r\n");
  }
  else
  {
    Dbg_Printf("cali: vernier z=%d\r\n", (int)(nv->mt_zero_rad * 1000.0f));
  }
  Pwm_EnableOutputs();
