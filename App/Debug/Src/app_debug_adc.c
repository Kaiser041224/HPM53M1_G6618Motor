...
void app_debug_adc_init(void) {
    ...
    app_debug_printf(
        "[ADC] chain: PWM1 CMP10 +%u ns -> TRGM PTRGI0A -> ADC0[IN2,IN3,IN4] PMT | ADC1[IN15,IN6,IN11,IN1] SEQ 1kHz(GPTMR0)\r\n",
        (unsigned) cfg->trigger_delay_ns);
    app_debug_printf("[ADC] %u bit / sample_cycle=%u / 触发 = PWM1 计数（25kHz）\r\n",
                     (unsigned) cfg->resolution, (unsigned) cfg->sample_cycle);
}

void app_debug_adc_dump_channels(void) {
    const app_adc_cfg_t *cfg = app_adc_get_config();

    app_debug_printf("[ADC] seq=%u valid=%u delay=%u ns\r\n", (unsigned) app_adc_get_sequence(),
    ...
    for (uint8_t ch = 0U; ch < (uint8_t) ADC_CH_COUNT; ch++) {
        ...
        if (ok) {
            app_debug_printf("[ADC] %-6s raw=%-5u %8.1f mV  %9.4f %s\r\n", s_ch_names[ch],
                             (unsigned) raw, (double) (app_adc_code_to_volts(raw) * 1000.0f),
                             (double) phys, s_ch_units[ch]);
        } else {
...
void app_debug_adc_dump_diag(void) {
    ...
    uint32_t dt_cycles = inited ? (cycles - last_cycle) : 0U;
    ...
    app_debug_printf("[ADC] seq=%u | dt=%u cyc\r\n", (unsigned) app_adc_get_sequence(),
                     (unsigned) dt_cycles);
    ...
    for (uint8_t inst = 0U; inst < (uint8_t) INTF_ADC_INSTANCE_COUNT; inst++) {
        ...
        if (dt_cycles > 0U) {
            uint32_t d = now.pmt_complete[inst] - last.pmt_complete[inst];
            uint64_t d_isr = now.isr_total_cycles[inst] - last.isr_total_cycles[inst];

            rate = (uint32_t) (((uint64_t) d * (uint64_t) intf_clock_get_cpu_freq()) / dt_cycles);
            isr_permille = (uint32_t) ((d_isr * 1000U) / dt_cycles);
        }

        app_debug_printf(
            "[ADC] ADC%u: irq=%u pmt=%u cb=%u drop=%u inv=%u(cyc=%u trg=%u ch=%u) fallback=%u isr_max=%u cyc isr_cpu=%u.%u%% rate=%u Hz\r\n",
    ...
    /* 关键寄存器状态（配置核对） */
    for (uint8_t inst = 0U; inst < (uint8_t) INTF_ADC_INSTANCE_COUNT; inst++) {
        app_debug_printf("[ADC] ADC%u regs: CONV_CFG1=0x%08X ADC_CFG0=0x%08X BUF_CFG0=0x%08X CONFIG0=0x%08X\r\n",
                         (unsigned) inst, (unsigned) now.reg_conv_cfg1[inst],
                         (unsigned) now.reg_adc_cfg0[inst], (unsigned) now.reg_buf_cfg0[inst],
                         (unsigned) now.reg_config0[inst]);
    }
    ...
    /* ADC1 慢通道：SEQ 运行状态与 PRD_RESULT（序列结果寄存器，手册 §53.2.4） */
    {
        ...
        app_debug_printf("[ADC] ADC1 sts: INT_STS=0x%08X SEQ_CFG0=0x%08X\r\n",
                         (unsigned) now.reg_int_sts[inst], (unsigned) now.reg_seq_cfg0[inst]);
        app_debug_printf("[ADC] ADC1 prd: ch6=%u ch11=%u ch1=%u ch15=%u\r\n",
    ...
    /* 最近一帧 PMT 原始字（仅 ADC0 为 PMT 模式）：bit31=cycle, [28:25]=trig, [24:20]=ch, [15:0]=result */
    {
        const uint8_t inst = 0U;

        app_debug_printf("[ADC] ADC%u raw:", (unsigned) inst);
        for (uint8_t i = 0U; i < 4U; i++) {
            uint32_t w = now.pmt_last[inst][i];

            app_debug_printf(" [cyc=%u trg=%u ch=%u seq=%u r=%u]", (unsigned) ((w >> 31) & 1U),
                             (unsigned) ((w >> 25) & 0xFU), (unsigned) ((w >> 20) & 0x1FU),
                             (unsigned) ((w >> 29) & 0x3U), (unsigned) (w & 0xFFFFU));
        }
        app_debug_printf("\r\n");
    }
