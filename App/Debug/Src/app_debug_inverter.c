...
void app_debug_inverter_set_output(uint8_t mask)
{
    ...
    app_debug_printf("[3PH] mask=0x%X [%s%s%s] %s\r\n", (unsigned) mask,
                     ((mask & 1U) != 0U) ? "U" : "-", ((mask & 2U) != 0U) ? "V" : "-",
                     ((mask & 4U) != 0U) ? "W" : "-", (mask != 0U) ? "ON" : "OFF");
}
...
void app_debug_inverter_init(void)
{
    ...
    (void) app_3phase_inverter_set_duty_abc(INVERTER_TEST_DUTY, INVERTER_TEST_DUTY, INVERTER_TEST_DUTY);
