...
void app_debug_usb_run_once(void) {
    ...
    if ((uint32_t)(now - s_last_tx_cycle) >= period_cycles) {
        ...
        s_tick++;
        len = snprintf(
            line, sizeof(line), "usb: tick=%u rx_total=%u\r\n", (unsigned)s_tick,
            (unsigned)s_rx_total);
        if (len > 0) {
