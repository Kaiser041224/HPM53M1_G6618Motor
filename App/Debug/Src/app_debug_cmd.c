...
static void cmd_print_info(void) {
    ...
    app_debug_printf(
        ...
        app_encoder_is_param_loaded() ? "valid" : "default", (unsigned)zero_rotor,
        (unsigned)zero_output, (unsigned)pos_rotor, (unsigned)pos_output);
}
...
void app_debug_cmd_handle(const uint8_t* data, size_t len) {
    ...
    for (size_t i = 0U; i < len; i++) {
        ...
        /* ADC 采样链 */
        case 'd': app_debug_adc_dump_channels(); break;
        case 'p': app_debug_adc_dump_diag(); break;
        case 'f': app_debug_fault_dump(); break;
        case 'F': app_debug_fault_clear(); break;
        case 'k': {
            /* 触发延时预设循环：验证采样点是否落在低侧导通窗口内 */
            static const uint32_t s_delay_presets[] = {100U, 250U, 500U, 1000U, 2000U};
            static uint8_t s_delay_index;
            uint32_t delay_ns = s_delay_presets[s_delay_index];

            s_delay_index = (uint8_t)((s_delay_index + 1U)
                                      % (sizeof(s_delay_presets) / sizeof(s_delay_presets[0])));
