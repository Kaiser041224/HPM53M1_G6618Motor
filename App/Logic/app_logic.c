...
void app_fast_step(void) {
     ...
     * 硬约束：无 RTOS API、无 printf、无动态分配、无等待。
     * 内容 = FOC 强实时必需：角度采样 → 换算 → 保护 → 控制输出。
     * 将来 FOC 控制环替换 app_debug_motor_run_once 的位置。
     * 角速度预算：转子 SPI ~7µs + 换算/保护 ~7µs + 查表 sin/cos + 输出 ~4µs
     * ≈ 18µs / 40µs（25kHz），余量充足。 */

    /* 1) 转子编码器采样（FOC 电角度反馈；25kHz 硬件触发节拍内） */
...
void app_io_step(void) {
    ...
    /* 编码器出轴采样（角度闭环阶段随角度环频率提速；转子已在 25kHz 快车道采样） */
