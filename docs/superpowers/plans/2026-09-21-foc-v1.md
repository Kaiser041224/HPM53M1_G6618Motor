...
**实现定稿修订（质量审查后，代码为准）**：
- 快照新增 `valid` / `fault_count`（保护路径：零矢量/输入无效 → `valid=false` + `fault_count++`）；
  快照改为全局 `g_foc_current_snapshot`（`.noncacheable.bss`，Ozone 直读，spec §4.2 要求）；
- `decoupling_en` 改为**每拍 live**（Algorithm 模块新增 `foc_current.set_decoupling()`）；
  其余 init 期参数（sample_time/l_d/l_q/lambda）语义已写入头文件；
- 反馈非有限 → 快照 `valid=false` 且 `i_d/i_q` 记 0；
- 移除未用的 `app_adc.h`；`v_max` 公式补推导注释。
...
| A2 | **必须先完成电角度标定**（`cal encoder` 或手填 `motor.encoder.electrical_offset_rad`/`direction`）→ `foc on` | `state=READY`；电流 ≈0；无异常噪声/发热。**未标定时严禁给转矩**：电流矢量将以任意电角度施加 → 空载电机飞转 → 高速失控 |
