...
**注意**：`g_enc_rotor_deg` 原实现可能带软件零点（`app_encoder_read_deg`），本次改为**原始角**（`raw` 直算）。若希望保留显示语义（零点修正），改为在 `app_encoder_get_rotor_raw` 之外另取零点做减法；**以 spec 为准：共享缓存为原始值**，Debug 显示原始角（`enc` 命令另有 `read_deg` 路径显示零点修正值）。
...
static bool app_foc_read_rotor_rad(float* theta_m_rad) {
    ...
    if (app_encoder_get_rotor_raw(&raw, &valid) != 0) {
        return false;
    }
    if (!valid) {
        return false;
...
int app_foc_enable(void) {
    ...
    if ((app_encoder_get_rotor_raw(&raw, &valid) != 0) || !valid) {
        return -1;
    }
