...
| 2026-09-21 | 台架修复（三轮） | **现象**：`foc on`（零给定）后转子抖动、电流 2.1s 内缓增至 14.5A → 跳闸；V/F 正常。**判读**：主循环执行的电流环在打印/终端停顿下有效延迟可达 3~5 拍（120~200µs），1kHz 目标带宽相位裕度不足 → 零给定下自激缓增。**修复/工具**：① 电流环默认带宽 1kHz → **500Hz**（kp 0.372 / ki 496.4；LIVE 可调；迁 ISR 后可回调）；② FOC 活动期间跳过 1Hz RTT 打印（打印停顿可达 ms 级，是最大延迟源）；③ 新增 **`foc vtest <v> [<deg>]`** 开环电压诊断（不经电流环，直接调制；用于核实采样符号/相序映射/标度与 v/R 直流增益）；④ 新增 **`foc trace`** 25kHz 波形捕获（128 拍 CSV：id,iq,vd,vq,theta） |
