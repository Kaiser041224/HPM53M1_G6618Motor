/*
 * CherryUSB 配置 —— HPM53M1_G6618Motor
 *
 * 基线：SDK samples/cherryusb/config/usb_config.h（保持完整 device/host 配置宏，
 *       usbotg_core.c 同时包含两套核心，缺宏会编译失败）。
 * 本项目差异：
 *   - 日志走 SEGGER RTT（与工程调试输出一致，ISR 上下文安全）
 *   - HPM53M1 集成 USB2.0 HS-PHY，默认高速；若枚举不稳可改全速：
 *     #define CONFIG_USB_DEVICE_FORCE_FULL_SPEED
 *
...
/* Enable print with color (RTT 终端不支持 ANSI 颜色，关闭) */
