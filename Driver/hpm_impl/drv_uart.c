 ...
 *         可选回调（intf_uart_register_rx_callback）在中断上下文执行
...
static int hpm_uart_transmit(intf_uart_port_t port, const uint8_t *data, size_t len,
                             uint32_t timeout_ms)
{
...
static int hpm_uart_receive(intf_uart_port_t port, uint8_t *data, size_t len,
                            uint32_t timeout_ms)
{
